...
namespace CL8.DAL.Repositories
{
    public class MessageRepository(MyContext myContext) : IRepository<Message>
    {
        ...
        public IQueryable<Message> GetAllEntities() => _ctx.Messages.AsNoTracking().AsQueryable();
        ...
        public async Task<IEnumerable<Message>> GetEntitiesAsync(Expression<Func<Message, bool>> predicate) => await GetAllEntities().AsNoTracking().Where(predicate).ToListAsync();
        public async Task<Message> GetEntityAsync(Expression<Func<Message, bool>> predicate) => await GetAllEntities().AsNoTracking().FirstOrDefaultAsync(predicate);
